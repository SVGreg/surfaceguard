...
|---|---|---|
| AST01 Malicious Skills | **Yes** — primary | code + `SKILL.md` prose patterns, including the agent-relayed-command (ClickFix) delivery path via `SG-INJ-011`, the falsified-completion-report path via `SG-INJ-012`, and the disarm-first precursor (host security controls switched off) via `SG-EXE-010` |
